subroutine byDiag()
	...
	double precision, parameter :: absThrsh = 1.0e-12
	double precision :: BoysFnl
	integer :: dimn, iter, boys, mode, occDimn
	...
	iter = boys(mcf, occDimn, dimn, absThrsh, BoysFnl, mode)
...
integer function boys(mcf, occDimn, dimn, absThrsh, BoysFnl, mode)	result(iter)
	use boysParams
	use externAccess
	...
	!	Clamp at static convergence point for now, iterlimit ~ 100 for 1e-6 (remember to checck signflip)
	integer, parameter :: iterlimit = 1e4
	logical :: scalConverged, isOrth, orthTest, isConverged, swid
	...
	do while(.not.(iter .and. scalConverged(iterGain, iterGainPrev, absThrsh)) .and. iter .lt. iterlimit)
		do i = start, end - 1
			...
			do j = i + 1, end
                ...
                A_ij = sum(LCL1eIND(i, j, :)**2) - 0.25*sum(	&
                    (LCL1eIND(i, i, :) - LCL1eIND(j, j, :))**2)
                B_ij = sum(LCL1eIND(i, j, :)*(LCL1eIND(i, i, :) - LCL1eIND(j, j, :)))
                ...
                long = cgamma*mcf(:, i) + sgamma*mcf(:, j) 
                lat = -sgamma*mcf(:, i) + cgamma*mcf(:, j)
				verifMat(:, i) = long
				verifMat(:, j) = lat
			end do
        end do
	...
	if(mode .eq. 2)	then
		spliter = boys(verifMat, occDimn, dimn, absThrsh, BoysFnl, 1)
		iter = iter + spliter
		print *, "Secondary localisation iterations: ", spliter
		return
	endif
    ...
    do i = 0, dimn**2/5 - 1
            write(*, 204)   (sprint(j), j = 5*i + 1, 5*(i + 1))
    end do
    write(*, 204) sprint(dimn**2)
